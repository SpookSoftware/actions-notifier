...
test.describe("Actions selectors", () => {
  ...
  for (const colorScheme of ["light", "dark"] as const) {
    test(`ACTION_RUNS_CONTAINER_SELECTOR selects the action container in ${colorScheme} mode`, async ({
      page,
    }) => {
      ...
      const matches = await page
        .locator(ACTION_RUNS_CONTAINER_SELECTOR)
        .count();
      expect(matches).toBe(1);
    });

    test(`ACTION_RUNS_SELECTOR selects the action runs in ${colorScheme} mode`, async ({
      page,
    }) => {
      await page.emulateMedia({ colorScheme });
      await page.goto("https://github.com/SpookSoftware/sandbox/actions");
    ...
    test(`ACTION_RUNS_SELECTOR does not select the parent element of the action runs in ${colorScheme} mode`, async ({
      page,
    }) => {
      await page.emulateMedia({ colorScheme });
      await page.goto("https://github.com/SpookSoftware/sandbox/actions");
    ...
    test(`SUCCESSFUL_SELECTOR selects successful action runs in ${colorScheme} mode`, async ({
      page,
    }) => {
      await page.emulateMedia({ colorScheme });
      await page.goto(
    ...
    test(`CURRENTLY_RUNNING_SELECTOR selects currently running actions in ${colorScheme} mode`, async ({
      page,
    }) => {
      await page.emulateMedia({ colorScheme });
...
test.describe("Job selectors", () => {
  for (const colorScheme of ["light", "dark"] as const) {
    test(`JOB_RUNS_CONTAINER_SELECTOR selects the container that has all job elements in ${colorScheme} mode`, async ({
      page,
    }) => {
      await page.emulateMedia({ colorScheme });
      await page.goto(
      ...
      const actualJobs = await jobRunsContainer
        .locator(JOB_RUN_SELECTOR)
        .count();
      expect(actualJobs).toBe(9);
    });

    test(`JOB_RUN_SELECTOR selects all the job runs in a runs page in ${colorScheme} mode`, async ({
      page,
    }) => {
      await page.emulateMedia({ colorScheme });
      await page.goto(
    ...
    test(`SUCCESSFUL_JOB_RUN_SELECTOR selects successful job runs in ${colorScheme} mode`, async ({
      page,
    }) => {
      await page.emulateMedia({ colorScheme });
      await page.goto(
    ...
    test(`FAILED_JOB_RUN_SELECTOR selects failed job runs in ${colorScheme} mode`, async ({
      page,
    }) => {
      await page.emulateMedia({ colorScheme });
      await page.goto(
