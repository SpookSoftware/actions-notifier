...
test.describe("Popup functionality", () => {
  test("should show warning state when no token exists", async ({ context, extensionId }) => {
  ...
  test("should save and validate GitHub token", async ({ context, extensionId }) => {
    // Open the popup
    const page = await context.newPage();
    await page.goto(`chrome-extension://${extensionId}/popup.html`);
    ...
    await page.route("https://api.github.com/repos/octocat/hello-world", async (route) => {
      await route.fulfill({
        status: 200,
    ...
    await expect(page.locator("#token-status")).toHaveText(/Token validated successfully/);
  ...
  test("should show error state for invalid token", async ({ context, extensionId }) => {
    // Open the popup
    const page = await context.newPage();
    await page.goto(`chrome-extension://${extensionId}/popup.html`);
    ...
    // Verify token status message
    await expect(page.locator("#token-status")).toHaveText(/Invalid token or insufficient permissions/);
    await expect(page.locator("#token-status")).toHaveClass(/token-invalid/);
  });

  test("should toggle extension enabled state", async ({ context, extensionId }) => {
    // Open the popup
    const page = await context.newPage();
    await page.goto(`chrome-extension://${extensionId}/popup.html`);
    ...
    await expect(page.locator("#extension-status")).toHaveCSS("color", "rgb(203, 36, 49)");
    ...
    await expect(page.locator("#extension-toggle")).toBeChecked();
    await expect(page.locator("#extension-status")).toHaveText("Enabled");
    await expect(page.locator("#extension-status")).toHaveCSS("color", "rgb(40, 167, 69)");
  });

  test("should open manage page when clicking manage button", async ({ context, extensionId }) => {
    // Open the popup
    const page = await context.newPage();
    await page.goto(`chrome-extension://${extensionId}/popup.html`);
    ...
    await page.route("https://api.github.com/repos/octocat/hello-world", async (route) => {
      await route.fulfill({ status: 200, body: JSON.stringify({ name: "hello-world" }) });
    });
  ...
  test("should open onboarding page when clicking debug button", async ({ context, extensionId }) => {
    // Open the popup
    const page = await context.newPage();
    await page.goto(`chrome-extension://${extensionId}/popup.html`);
  ...
  test("should show welcome message for first-time users", async ({ context, extensionId }) => {
    // Open the popup
    const page = await context.newPage();
