<script lang="ts">
  ...
  export let onFinish: () => Promise<void>;
  // Pass in value from outside, but fall back to our own check if not provided
  export let isPaidOrTrialing: boolean | undefined = undefined;
...
<div class="controls">
  <div class="buttons-container">
    {#if currentStep > 1}
      <button class="btn btn-secondary" on:click={onPrevious}>
        Previous
      </button>
    {/if}
    ...
    {#if isLastStep}
      <div style="position: relative">
        <button 
          ...
          title={!effectivePaidStatus ? "Please start a trial or make a purchase to continue" : ""}
        >
          Get Started
        </button>
        
        {#if !effectivePaidStatus}
          <div style="position: absolute; top: -25px; left: 50%; transform: translateX(-50%); background-color: #e25822; color: white; padding: 3px 8px; border-radius: 4px; font-size: 12px; white-space: nowrap">
